import { useState, useEffect } from "react";

function TimeSelector() {
  const timesArray = [
    "00:00",
    "00:30",
    "01:00",
    "01:30",
    "02:00",
    "02:30",
    "03:00",
    "03:30",
    "04:00",
    "04:30",
    "05:00",
    "05:30",
    "06:00",
    "06:30",
    "07:00",
    "07:30",
    "08:00",
    "08:30",
    "09:00",
    "09:30",
    "10:00",
    "10:30",
    "11:00",
    "11:30",
    "12:00",
    "12:30",
    "13:00",
    "13:30",
    "14:00",
    "14:30",
    "15:00",
    "15:30",
    "16:00",
    "16:30",
    "17:00",
    "17:30",
    "18:00",
    "18:30",
    "19:00",
    "19:30",
    "20:00",
    "20:30",
    "21:00",
    "21:30",
    "22:00",
    "22:30",
    "23:00",
    "23:30",
  ];
  const [busyArray, setBusyArray] = useState([]);
  const [startDate, setStartDate] = useState(
    window.sessionStorage.getItem("storedDate")
  );
  let busy = [];

  useEffect(() => {
    getBusyTimes();
    setStartDate(window.sessionStorage.getItem("storedDate"));
  }, [busy]);

  async function getBusyTimes() {
    await fetch("http://localhost:3002/availableTimes")
      .then((res) => res.json())
      .then((res) => setBusyArray(res));
  }

  function setTime(e) {
    window.sessionStorage.setItem("time", e.target.value);
  }

  //startDate = window.sessionStorage.getItem('storedDate')

  //console.log("Busy Array: ", busyArray)

  busyArray.map((events) => {
    if (startDate === events.startDate) {
      busy.push(events.startTime);
    }
  });
  //console.log("Busy aka Start Time: ", busy)

  return (
    <div className="container mt-3 d-flex flex-wrap justify-content-around ">
      {timesArray.map((time, index) => (
        <div className="mt-1 time-selector">
          <input
            value={index * 30}
            type="radio"
            className="btn-check"
            name="times"
            id={time}
            onClick={setTime}
          />

          <label
            style={{
              backgroundColor: busy.includes(time) ? "#9a0000" : "#008000",
              pointerEvents: busy.includes(time) ? "none" : "auto",
            }}
            className="btn btn-time"
            for={time}
          >
            {time}
          </label>
        </div>
      ))}
    </div>
  );
}
export default TimeSelector;

// style={{backgroundColor:
// time.available === 'true' ? '#008000' : '#9a0000' }}
